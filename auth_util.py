...
def generate_pair():
    """Generates a public/private key pair using the crypto library, the returns them as base64 strings."""
...
def generate_binary_key(b64size):
    ...
    """Generates a random binary key given the length of the base64 encoded string that will be produced."""
...
def generate_nonce():
    ...
    nonce_str = base64.b64encode(generate_binary_key(32))

    return nonce_str

def decrypt_auth_token(tokenFromPayload, public_key_str, private_key_str):
    """Retrieves the auth token and decrypt it, in a way that does not specify the name of the service."""
